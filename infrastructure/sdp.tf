provider "azurerm" {
  ...
  alias                      = "mi_vault"
  subscription_id            = local.sdp_environment_ids[local.bspayment_environment].subscription
}
...
locals {
  ...
  bspayment_environment = lookup(local.sdp_cft_environments_map, var.env, var.env)
...
module "sdp_db_user" {
  ...
  env    = local.bspayment_environment

  server_name       = "${var.product}-${var.component}-postgres-db-v15-${var.env}"
  server_fqdn       = module.ccpay-bulkscanning-payment-database-v15.fqdn
  server_admin_user = module.ccpay-bulkscanning-payment-database-v15.username
  server_admin_pass = module.ccpay-bulkscanning-payment-database-v15.password
